...
const Testimonials = () => {
        ...
        const handleScroll = (direction: 'left' | 'right') => {
            ...
            const cardWidth =
                viewportWidth >= 1280 ? 400 :
                    viewportWidth >= 1024 ? 360 :
                        viewportWidth >= 640 ? 340 :
                            320;
            const gap = 24; // gap-6
        ...
        return (
            <section className="relative w-full overflow-hidden">
                ...
                <div className="relative w-full mt-10 md:mt-16">
                    ...
                    <div
                        className="absolute left-0 top-0 bottom-0 w-6 sm:w-12 xl:w-24 bg-linear-to-r from-background via-background/60 to-transparent backdrop-blur-[2px] z-10 pointer-events-none"/>

                    {/* Right Blur Fade Overlay */}
                    <div
                        className="absolute right-0 top-0 bottom-0 w-6 sm:w-12 xl:w-24 bg-linear-to-l from-background via-background/60 to-transparent backdrop-blur-[2px] z-10 pointer-events-none"/>
                        ...
                        <div className="flex gap-6 w-max animate-marquee-track pause-on-hover">
                            {tripleData.map((testimonial, idx) => (
                                <ReviewCard
                                    ...
                                    // Setting explicit width targets via arbitrary values to keep them uniform
                                    className="w-80 sm:w-85 lg:w-90 xl:w-100 shrink-0"
                                />
                            ))}
                        </div>
                    </div>
